import"./config-HRWLmo66.js";import"./logger-DHpG8Bim.js";import{parseDate as e}from"./parse-date-DHsdom8D.js";import{ofetch_default as t}from"./ofetch-DRl42yaJ.js";import{timezone as n}from"./timezone-BrxBCotj.js";import{ViewType as r}from"./types-D9T3SQ-7.js";import{load as i}from"cheerio";const a=async r=>{let{category:a,id:o}=r.req.param(),s=Number.parseInt(r.req.query(`limit`)??`30`,10),c=`http://load.grainoil.com.cn`,l=new URL(`${a}/${o}.jspx`,c).href,u=await t(l),d=i(u),f=d(`html`).attr(`lang`)??`zh-CN`,p=[];return p=d(`div.m_listpagebox ol li a`).slice(0,s).toArray().map(t=>{let r=d(t),i=r.find(`b`).text(),a=r.find(`span`).text().trim(),o=r.attr(`href`),s=a,c={title:i,pubDate:a?n(e(a),8):void 0,link:o,updated:s?n(e(s),8):void 0,language:f};return c}),p=(await Promise.all(p.map(e=>e.link?cache.tryGet(e.link,async()=>{let n=await t(e.link),r=i(n),a=r(`div.m_tit h2`).text(),o=r(`div.TRS_Editor`).html()??``,s=r(`div.m_tit h2 a`).first().text(),c={title:a,description:o,author:s,content:{html:o,text:o},language:f};return{...e,...c}}):e))).filter(e=>!0),{title:d(`title`).text(),description:d(`meta[http-equiv="description"]`).attr(`content`),link:l,item:p,allowEmpty:!0,image:d(`div.top_logo img`).attr(`src`)?new URL(d(`div.top_logo img`).attr(`src`),c).href:void 0,author:d(`meta[http-equiv="keywords"]`).attr(`content`),language:f,id:l}},o={path:`/:category/:id`,name:`分类`,url:`load.grainoil.com.cn`,maintainers:[`nczitzk`],handler:a,example:`/grainoil/newsListHome/3`,parameters:{category:{description:"分类，默认为 `newsListHome`，可在对应分类页 URL 中找到",options:[{label:`newsListHome`,value:`newsListHome`},{label:`newsListChannel`,value:`newsListChannel`}]},id:{description:`分类 ID，可在对应分类页 URL 中找到`}},description:`:::tip
