...
</details>`};async function c(s){let{category:c=``}=s.req.param(),l=s.req.query(`limit`)?Number.parseInt(s.req.query(`limit`),10):50,u=`https://www.thoughtco.com`,d=new URL(c,u).href,{data:f}=await i(d),p=o(f),m=p(`a[data-doc-id]`).slice(0,l).toArray().map(e=>(e=p(e),{title:e.find(`span.block-title`).text(),link:new URL(e.prop(`href`),u).href}));m=await Promise.all(m.map(s=>e.tryGet(s.link,async()=>{let{data:e}=await i(s.link),c=o(e);return c(`div.adslot`).remove(),c(`div.sources-and-citation, .mntl-figure-caption svg`).remove(),c(`div.figure-media`).each((e,n)=>{n=p(n);let i=n.find(`img`);n.replaceWith(t(a.join(r,`templates/description-87496755.art`),{image:{src:i.prop(`data-src`),width:i.prop(`width`),height:i.prop(`height`)}}))}),s.title=c(`meta[property="og:title"]`).prop(`content`),s.description=t(a.join(r,`templates/description-87496755.art`),{image:{src:c(`meta[property="og:image"]`).prop(`content`)},description:c(`div.article-content`).html()}),s.author=c(`meta[name="sailthru.author"]`).prop(`content`),s.category=[...new Set(c(`meta[name="parsely-tags"]`).prop(`content`)?.split(/,/).map(e=>e.trim()))],s.pubDate=n(e.match(/"datePublished": "(.*?)"/)[1]),s.updated=n(e.match(/"dateModified": "(.*?)"/)[1]),s})));let h=p(`meta[property="og:site_name"]`).prop(`content`),g=p(`meta[property="og:title"]`).prop(`content`),_=new URL(p(`link[rel="apple-touch-icon-precomposed"]`).prop(`href`),u).href;return{item:m,title:`${h}${g.startsWith(h)?``:` - ${g}`}`,link:d,description:p(`meta[property="og:description"]`).prop(`content`),language:p(`html`).prop(`lang`),image:p(`meta[property="og:image"]`).prop(`content`),icon:_,logo:_,subtitle:p(`meta[property="og:title"]`).prop(`content`),author:h}}export{s as route};