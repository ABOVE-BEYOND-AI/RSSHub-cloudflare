import"./config-HRWLmo66.js";import"./logger-DHpG8Bim.js";import{cache_default as e}from"./cache-C3AIQtoX.js";import{art as t}from"./render-DE4LRFBD.js";import{parseDate as n}from"./parse-date-DHsdom8D.js";import{ofetch_default as r}from"./ofetch-DRl42yaJ.js";import{__dirname as i}from"./esm-shims-BDPl6Msv.js";import{ViewType as a}from"./types-D9T3SQ-7.js";import o from"node:path";import s from"node:crypto";const c={path:`/:user/:type?`,categories:[`design`],view:a.Pictures,example:`/behance/mishapetrick`,parameters:{user:`username`,type:{description:`type`,options:[{value:`projects`,label:`projects`},{value:`appreciated`,label:`appreciated`}],default:`projects`}},features:{requireConfig:!1,requirePuppeteer:!1,antiCrawler:!1,supportBT:!1,supportPodcast:!1,supportScihub:!1},name:`User Works`,maintainers:[`MisteryMonster`],handler:u,description:"Behance user's profile URL, like [https://www.behance.net/mishapetrick](https://www.behance.net/mishapetrick) the username will be `mishapetrick`。"},l=async(t,n)=>await e.tryGet(`behance:profile:${n}`,()=>{let e=t.flatMap(e=>e.owners).find(e=>e.username===n);return Promise.resolve({displayName:e.displayName,id:e.id,link:e.url,image:e.images.size_50.url.replace(`/user/50/`,`/user/source/`)})});async function u(a){let{user:c,type:u=`projects`}=a.req.param(),d=s.randomUUID(),f={Cookie:`gk_suid=${Math.random().toString().slice(2,10)}, gki=; originalReferrer=; bcp=${d}`,"X-BCP":d,"X-Requested-With":`XMLHttpRequest`},p=await r(`https://www.behance.net/v3/graphql`,{method:`POST`,headers:f,body:{query:u===`projects`?`query GetProfileProjectsAndSections($username: String, $after: String) {
  ...
  }`,variables:{projectId:a.projectId}}}),s=e.data.project;return a.description=t(o.join(i,`templates/description-69dc492c.art`),{description:s.description,modules:s.allModules}),a.category=[...new Set([...a.category||[],...s.tags?.map(e=>e.title.toLowerCase())||[]])],a.pubDate=a.pubDate||(s.publishedOn?n(s.publishedOn,`X`):void 0),a})));return{title:`${g.displayName}'s ${u}`,link:`https://www.behance.net/${c}/${u}`,image:g.image,item:_}}export{c as route};