import"./config-HRWLmo66.js";import"./logger-DHpG8Bim.js";import"./helpers-LVq640iW.js";import{art as e}from"./render-DE4LRFBD.js";import{parseDate as t}from"./parse-date-DHsdom8D.js";import"./ofetch-DRl42yaJ.js";import{__dirname as n}from"./esm-shims-BDPl6Msv.js";import{got_default as r}from"./got-BaOFZRd4.js";import i from"node:path";import{load as a}from"cheerio";const o=`https://xgzlyhd.samr.gov.cn`,s=new URL(`gjjly/message/getMessageList`,o).href,c=new URL(`gjjly/message/getDataList`,o).href,l=new URL(`gjjly/index`,o).href,u={category:`1`,department:`2`},d=async e=>{let{data:t}=await r.post(c,{json:{type:u[e]}});return t.data},f=async(e,t)=>{let n=await d(e),r=n.filter(e=>e.name===t||e.code===t);if(r.length>0)return r.pop()},p={path:`/samr/xgzlyhd/:category?/:department?`,categories:[`government`],example:`/gov/samr/xgzlyhd`,parameters:{category:`留言类型，见下表，默认为全部`,department:`回复部门，见下表，默认为全部`},features:{requireConfig:!1,requirePuppeteer:!1,antiCrawler:!1,supportBT:!1,supportPodcast:!1,supportScihub:!1},radar:[{source:[`xgzlyhd.samr.gov.cn/gjjly/index`]}],name:`留言咨询`,maintainers:[`nczitzk`],handler:m,url:`xgzlyhd.samr.gov.cn/gjjly/index`,description:`#### 留言类型
...
| 离退办                       | 6bf265ffd1c94fa4a3f1687b03fa908b |`};async function m(c){let{category:u,department:d}=c.req.param(),p=c.req.query(`limit`)?Number.parseInt(c.req.query(`limit`),10):10,m,h;u&&(m=await f(`category`,u)),d&&(h=await f(`department`,d));let{data:g}=await r.post(s,{json:{clyj:``,curPage:`1`,endTime:``,fzsj:h?.code??``,lybt:void 0,lylx:m?.code??``,lynr:``,startTime:``,zj:``}}),_=g.data.data.slice(0,p).map(r=>({title:r.lybt,link:`${l}#${r.zj}`,description:e(i.join(n,`templates/description-5c2bbb00.art`),{item:r}),author:`${r.lyr} ⇄ ${r.fzsjCn}`,category:[r.fzsjCn],guid:`${l}#${r.zj}`,pubDate:t(r.pubtime)})),{data:v}=await r(l),y=a(v),b=`国家市场监督管理总局`,x=y(`title`).text(),S=[m?m.name:void 0,h?h.name:void 0].filter(Boolean).join(` - `),C=new URL(y(`link[rel="icon"]`).prop(`href`),o).href;return{item:_,title:`${b}${x}${S?` - ${S}`:``}`,link:l,description:y(`meta[property="og:description"]`).prop(`content`),language:`zh`,image:new URL(`gjjly/${y(`div.fd-logo img`).prop(`src`)}`,o).href,icon:C,logo:C,subtitle:S,author:b,allowEmpty:!0}}export{p as route};