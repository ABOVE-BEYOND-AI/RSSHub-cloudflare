...
</details>`};async function l(c){let{industry:l,label:u}=c.req.param(),d=c.req.query(`limit`)?Number.parseInt(c.req.query(`limit`),10):50,f=`https://www.questmobile.com.cn`,p=new URL(`api/v2/report/article-list`,f).href,m=new URL(`api/v2/report/industry-label-tree`,f).href,{data:{data:{industryTree:h,labelTree:g}}}=await i(m),_=s(h),v=s(g),y=l?_.find(e=>e.key===l||e.value===l):void 0,b=u?v.find(e=>e.key===u||e.value===u):y?void 0:v.find(e=>e.key===l||e.value===l),x=y?.key??-1,S=b?.key??-1,C=new URL(`research/reports/${y?.key??-1}/${b?.key??-1}`,f).href,{data:w}=await i(p,{searchParams:{version:0,pageSize:d,pageNo:1,industryId:x,labelId:S}}),T=w.data.slice(0,d).map(e=>({title:e.title,link:new URL(`research/report/${e.id}`,f).href,description:t(a.join(r,`templates/description-32f758d3.art`),{image:{src:e.coverImgUrl,alt:e.title},introduction:e.introduction,description:e.content}),category:[...e.industryList??[],...e.labelList??[]],guid:`questmobile-report#${e.id}`,pubDate:n(e.publishTime)}));T=await Promise.all(T.map(s=>e.tryGet(s.link,async()=>{let{data:e}=await i(s.link),c=o(e);return c(`div.text div.daoyu`).remove(),s.title=c(`div.title h1`).text(),s.description+=t(a.join(r,`templates/description-32f758d3.art`),{description:c(`div.text`).html()}),s.author=c(`div.source`).text().replace(/^.*?：/,``),s.category=c(`div.hy, div.keyword`).find(`span`).toArray().map(e=>c(e).text()),s.pubDate=n(c(`div.data span`).prop(`datetime`)),s})));let{data:E}=await i(C),D=o(E),O=D(`meta[property="og:title"]`).prop(`content`).split(/-/)[0],k=[y?.value,b?.value].filter(Boolean),A=D(`img[alt="${O}"]`).prop(`src`),j=D(`link[rel="shortcut icon"]`).prop(`href`);return{item:T,title:`${O}${k.length===0?``:` - ${k.join(` - `)}`}`,link:C,description:D(`meta[property="og:description"]`).prop(`content`),language:`zh`,image:A,icon:j,logo:j,subtitle:D(`meta[name="keywords"]`).prop(`content`),author:O,allowEmpty:!0}}export{c as route};